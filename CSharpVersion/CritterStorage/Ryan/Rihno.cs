...
{
    public class Rihno : Shell
    {
        private readonly int SPIN = 20;
        private readonly int PAUSE_FOR = 150;
        private readonly int PAUSE_EVERY = 400;
        ...
        public override Critter.Action GetMove(ICritterInfo info)
        {
            ...
            if (doPause)
            {
                ...
                else if (pauseStep <= PAUSE_FOR && !hasWall && !(neighbor[front] == same && neighbor[back] == same))
                {
                    if (threat[right])
                    {
                        return R;
